  ...
  }={})
{
    ...
    var result = line_format_str + line.repeat(Number.parseInt(size / line.length)) + line.slice(0, size % line.length) + "\033[0m\n";
    for (const title_line of txt.split('\n'))
        result += txt_format_str + center(title_line, size, ' ') + "\033[0m\n";
    result += line_format_str + line.repeat(Number.parseInt(size / line.length)) + line.slice(0, size % line.length) + "\033[0m";
    return result;
}
  ...
  }={})
{
    ...
    var result = line_format_str + center(' '+ title +' ', size, line) + "\033[0m\n";
    for (const message_line of txt.split('\n'))
        result += txt_format_str + center(message_line, size, ' ') + "\033[0m\n";
    result += line_format_str + line.repeat(Number.parseInt(size / line.length)) + line.slice(0, size % line.length) + "\033[0m";
    return result;
}
